...
public class ListDbAdapter {
    ...
    public double getSum(){
        ...
        double res1=0;
        double res2=0;
        String query = "SELECT " + KEY_PRICE + ", "
                + KEY_DESCRIPTION+ " FROM " + DATABASE_TABLE;
        Cursor cursor = database.rawQuery(query, null);
        cursor.moveToFirst();
        while(cursor.isAfterLast() == false){
            res1 = Double.valueOf(cursor.getString(cursor
                    .getColumnIndexOrThrow(KEY_DESCRIPTION)));
            res2 = Double.valueOf(cursor.getString(cursor
